...
from env import WATATOKEN

# Импорт из src
from wataproclient import WataClientManager, WataClient, ApiResourceNotFoundError, ApiError

WataClient()

WataClientManager.register("prod")
...
async def client_manager_example():
    """Пример использования менеджера клиентов WATA API."""
    # Загрузка переменных окружения из файла .env
    load_dotenv()
    ...
    print("\n" + "=" * 80)
    print("ПРИМЕР ИСПОЛЬЗОВАНИЯ МЕНЕДЖЕРА КЛИЕНТОВ WATA API")
    print("=" * 80)
    
    # Получение учетных данных API из переменных окружения
    jwt_token = os.getenv("WATA_JWT_TOKEN", "ваш-токен-здесь")
    
    # Создание и регистрация клиентов
    print("\nСоздание и регистрация клиентов...")
    
    # Создание клиента для песочницы
    ...
    print("Клиент 'sandbox' успешно создан и зарегистрирован")
    
    # Создание клиента для продакшн среды
    WataClientManager.create(
        "production",
    ...
    print("Клиент 'production' успешно создан и зарегистрирован")
    
    try:
        # Пример 1: Получение клиента и работа с ним
        print("\n1. Получение клиента из менеджера и работа с ним...")
        sandbox_client = WataClientManager.get("sandbox")
        
        # Получение публичного ключа
        try:
            public_key = await sandbox_client.payments.get_public_key()
            print(f"Публичный ключ получен через менеджер: {public_key.get('key', 'Ключ не найден')[:30]}...")
        except Exception as e:
            print(f"Ошибка при получении публичного ключа: {e}")
            
        # Пример 2: Параллельное использование нескольких клиентов
        print("\n2. Параллельное использование нескольких клиентов...")
        
        # Определение асинхронной функции для создания платежа
        async def create_test_payment(client_name, order_id, amount):
            try:
                client = WataClientManager.get(client_name)
                payment_link = await client.links.create(
                    amount=amount,
                    currency="RUB",
                    order_id=order_id,
                    description=f"Тестовый платеж через менеджер ({client_name})",
                    success_url="https://example.com/success",
                    fail_url="https://example.com/fail"
                )
                return payment_link.get('url')
            except Exception as e:
                print(f"Ошибка при создании платежа {order_id}: {e}")
                return None
        
        # Создание платежей параллельно с использованием разных клиентов
        tasks = [
            create_test_payment("sandbox", "MANAGER-TEST-1", 100.00),
            create_test_payment("sandbox", "MANAGER-TEST-2", 200.00),
            create_test_payment("production", "MANAGER-PROD-1", 300.00)
        ]
        ...
        print(f"Успешно создано платежей: {sum(1 for r in results if r)} из {len(tasks)}")
        print(f"Время выполнения: {(end_time - start_time).total_seconds():.2f} секунд")
        
        for i, url in enumerate(results):
            if url:
                print(f"Платеж #{i+1}: {url}")
        
        # Пример 3: Проверка существования клиентов
        print("\n3. Проверка существования клиентов...")
        print(f"Существует ли клиент 'sandbox'? {WataClientManager.exists('sandbox')}")
        print(f"Существует ли клиент 'test'? {WataClientManager.exists('test')}")
        
    finally:
        # Закрытие всех клиентов
        print("\nЗакрытие всех клиентов...")
        await WataClientManager.close_all()
        print("Все клиенты успешно закрыты")
    
    print("\nПример использования менеджера клиентов WATA API завершен!")
...
async def main():
    ...
    # Запуск примера с использованием менеджера клиентов
    await client_manager_example()
    
    # Запуск примера проверки подписи вебхука
    await webhook_verification_example()
